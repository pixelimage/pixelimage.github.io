...
var CMS_GuideView = (function(){
	...
	function createlayout_core(_callback){
		...
		v.btn_zoom.click(function(){openEx()})
		//
		try{
			view.draggable({handle: view.find('.dragBarArea')});
		}catch( e ){}
		_callback();
	}
	
	function setBtn(){
		$("body").on("click",".btn_guide_block",function(event){
			var id = $(this).data("id");
			stageIn(id);
	...
	function getGloupHTML(_gloup){
		...
		var id = _gloup.attr("id");
			...
			items.each(function(){
				...
				if(_id == ""){
					tag += '</ul><p class="_h4">' + item.attr("name") + '</p><ul>';
				} else{
					if(nn){
						var s = id + '/' + _id;
	...
	function update(_id){
		...
		v.naviItems.removeClass("current");
		var aa = _id.split("/");
		v.navi.find("#"+getUID(aa[0],aa[1])).addClass("current");
